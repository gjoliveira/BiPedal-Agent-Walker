...
def train_model(algo, algo_name, policy, n_envs=os.cpu_count(), use_gpu=False):
    ...
    # Criar ambiente
    env = make_vec_env(env_id, n_envs=n_envs, vec_env_cls=SubprocVecEnv, vec_env_kwargs=dict(start_method='fork'))
...
def main():
    try:
        ...
        algorithms = [
            ...
            {"algo": PPO, "name": "PPO", "policy": "MlpPolicy", "n_envs": 150, "use_gpu": False},   # CPU
            ...
            #{"algo": TRPO, "name": "TRPO", "policy": "MlpPolicy", "n_envs": 50, "use_gpu": False},   # GPU
        ]
